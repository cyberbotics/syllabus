...
const char *challenge_start_position[] = {"-2.05 8.28 0", "-6.1 8.28 0",
                                          "-7.78 -0.4 0", "-2.17 0.44 0"};
double challenge_target_position[4][2] = {
    {-1.81, 3.98}, {-6, 0.39}, {-7.85, -6.2}, {-1.5, -4.37}};

double viewpoint_orient[4][4] = {{ 0.191, 0.129, -0.973, 1.98}, { 0.441, 0.221, -0.87, 2.32},
                                { 0.187, 0.101, -0.977, 2.17}, { 0.173, 0.232, -0.957, 1.32}};
double viewpoint_pos[4][3] = {{ 0.166, 11.9, 3.14 }, { -2.89, 9.5, 8.49 },
                              { -4.01, 4.15, 4.04 }, { -4.44, 4.9, 4.56 }};
...
void wb_robot_window_step(int time_step) {
  ...
  if (message) {
    if (strncmp(message, "robotName:", 10) == 0) {
      ...
      // load the robot in front of the given challenge number with its controller
      ...
      wb_supervisor_field_import_mf_node_from_string(children_field, -1, robot_node_content);
      
      // move the viewpoint to the challenge
      WbNodeRef viewpoint_node = wb_supervisor_node_get_from_def("VIEWPOINT");
      WbFieldRef viewpoint_orient_field = wb_supervisor_node_get_field(viewpoint_node, "orientation");
      WbFieldRef viewpoint_pos_field = wb_supervisor_node_get_field(viewpoint_node, "position");
      wb_supervisor_field_set_sf_rotation(viewpoint_orient_field, viewpoint_orient[challenge_number]);
      wb_supervisor_field_set_sf_vec3f(viewpoint_pos_field, viewpoint_pos[challenge_number]);
  ...
  if (play_simulation) {
    ...
    if (count > 500) { //after 500 robot step we consider that the robot is not moving anymore.
      play_simulation = false;
